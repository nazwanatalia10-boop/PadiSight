...
img_base64 = get_img_base64("WhatsApp Image 2026-05-26 at 08.18.23.jpeg")
...
<style>
  ...
  /* Kurangi padding atas bawaan Streamlit */
  .block-container {
...
# Inject sidebar background secara terpisah
...
if menu == "🏠 Beranda":
    st.markdown("""
        <div style='padding:4px 0 8px'>
            ...
            <h1 style='font-size:2rem; font-weight:800; letter-spacing:-0.5px; margin:6px 0'>
                Prediksi <span style='color:#1a6b3c'>Produktivitas</span> Padi Indonesia
            </h1>
            <p style='color:#4a6355; font-size:0.92rem; max-width:520px; line-height:1.6; margin:0 0 16px'>
    ...
    with c1:
        st.markdown("""
        <div class='padisight-card' style='margin-top:16px'>
            <div style='font-size:1.8rem; margin-bottom:10px'>🌾</div>
            <div style='font-size:0.72rem; font-weight:600; color:#2d9e5f; letter-spacing:1px; margin-bottom:4px'>01 — PREDIKSI</div>
            <h3 style='margin:0 0 6px; font-size:1.1rem'>Prediksi Produktivitas</h3>
            <p style='color:#4a6355; font-size:0.85rem; line-height:1.6; margin:0'>
    ...
    with c2:
        ...
        <div class='padisight-card' style='margin-top:16px'>
            <div style='font-size:1.8rem; margin-bottom:10px'>📈</div>
            <div style='font-size:0.72rem; font-weight:600; color:#2d9e5f; letter-spacing:1px; margin-bottom:4px'>02 — PERAMALAN</div>
            <h3 style='margin:0 0 6px; font-size:1.1rem'>Peramalan Tren</h3>
            <p style='color:#4a6355; font-size:0.85rem; line-height:1.6; margin:0'>
...
# ══════════════════════════════════════════════════════════
    ...
    st.markdown("""
        ...
        <h1 style='font-size:1.8rem; font-weight:800; letter-spacing:-0.5px; margin:4px 0'>Prediksi Produktivitas Padi</h1>
        <p style='color:#4a6355; margin-bottom:16px; font-size:0.9rem'>Masukkan data iklim suatu provinsi untuk mendapatkan prediksi produktivitas, analisis kondisi iklim, dan rekomendasi tindakan.</p>
    """, unsafe_allow_html=True)

    col_form, col_hasil = st.columns([1, 1.2], gap="large")

    with col_form:
        st.markdown("<p style='font-size:0.95rem; font-weight:700; color:#0f1c14; margin-bottom:4px'>📥 Input Data Iklim</p>", unsafe_allow_html=True)
        ...
        st.markdown("<p style='font-size:0.75rem; font-weight:600; color:#4a6355; margin:8px 0 4px; letter-spacing:0.5px'>OPSIONAL — HITUNG PRODUKSI TOTAL</p>", unsafe_allow_html=True)
    ...
    with col_hasil:
        if prediksi_btn:
            ...
            <div style='margin-top:12px; padding-top:12px; border-top:1px solid rgba(255,255,255,0.08)'>
                ...
                <div style='font-size:1.4rem; font-weight:700; color:#e8c84a'>{produksi:,.0f} ton</div>
            </div>""" if produksi else ""

            st.markdown(f"""
            <div style='background:#0f1c14; border-radius:20px; padding:24px; text-align:center; margin-bottom:12px'>
                <div style='font-size:0.7rem; font-weight:500; color:#5ac98a; letter-spacing:1px; text-transform:uppercase; margin-bottom:4px'>Produktivitas Prediksi</div>
                <div style='font-size:2.8rem; font-weight:800; color:white; letter-spacing:-2px; line-height:1'>
                    {produktivitas:.2f} <span style='font-size:0.95rem; color:rgba(255,255,255,0.5); font-weight:400'>kuintal/ha</span>
                </div>
                <div style='font-size:0.8rem; color:rgba(255,255,255,0.4); margin-top:4px'>{provinsi.capitalize()} · {tahun}</div>
                <div style='margin-top:10px; display:inline-block; background:{kat["warna"]}30; color:{kat["warna"]}; padding:5px 14px; border-radius:100px; font-size:0.8rem; font-weight:600'>
            ...
            with c1:
                if avg_prov.get('Produktivitas'):
                    ...
                    <div class='padisight-card'>
                        <div style='font-size:0.88rem; font-weight:700; color:#0f1c14; margin-bottom:12px'>📊 Perbandingan Historis</div>
                        <table style='width:100%; font-size:0.8rem; border-collapse:collapse'>
                            <tr><td style='color:#4a6355; padding:6px 0; border-bottom:1px solid rgba(26,107,60,0.06)'>Prediksi saat ini</td><td style='text-align:right; font-weight:600'>{produktivitas:.2f} kw/ha</td></tr>
                            <tr><td style='color:#4a6355; padding:6px 0; border-bottom:1px solid rgba(26,107,60,0.06)'>Rata-rata historis provinsi</td><td style='text-align:right; font-weight:600'>{avg_prov['Produktivitas']:.2f} kw/ha</td></tr>
                            <tr><td style='color:#4a6355; padding:6px 0; border-bottom:1px solid rgba(26,107,60,0.06)'>Rata-rata nasional</td><td style='text-align:right; font-weight:600'>{avg_nasional:.2f} kw/ha</td></tr>
                    ...
                    st.markdown("<div class='padisight-card'><div style='font-size:0.88rem; font-weight:700; color:#0f1c14; margin-bottom:8px'>📊 Perbandingan Historis</div><p style='font-size:0.8rem; color:#4a6355'>Data historis belum tersedia.</p></div>", unsafe_allow_html=True)
            ...
            with c2:
                ...
                for f, v in status.items():
                    ...
                    status_rows += f"""
                    <div style='display:flex; align-items:center; gap:8px; padding:6px 10px; background:#f5f0e8; border-radius:8px; margin-bottom:5px'>
                        <div style='width:7px; height:7px; border-radius:50%; background:{v["warna"]}; flex-shrink:0'></div>
                        <div style='flex:1; font-size:0.78rem; color:#4a6355'>{LABEL_IKLIM[f]}</div>
                        <div>
                            <div style='font-size:0.72rem; font-weight:600; color:{v["warna"]}'>{v["label"]}</div>
                            <div style='font-size:0.68rem; color:#4a6355'>{sp_str} dari historis</div>
                        </div>
                    </div>"""
                st.markdown(f"""
                <div class='padisight-card'>
                    <div style='font-size:0.88rem; font-weight:700; color:#0f1c14; margin-bottom:12px'>🌡️ Status Kondisi Iklim</div>
                    {status_rows}
                </div>
                """, unsafe_allow_html=True)
            ...
            st.markdown("""
            <div style='text-align:center; padding:40px 20px; color:#4a6355'>
                <div style='font-size:2.5rem; margin-bottom:10px'>🌾</div>
                <p style='font-size:0.9rem'>Isi form di kiri dan klik<br><strong>"Prediksi Sekarang"</strong></p>
            </div>
            """, unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════
    ...
    st.markdown("""
        ...
        <h1 style='font-size:1.8rem; font-weight:800; letter-spacing:-0.5px; margin:4px 0'>Peramalan Tren Produktivitas</h1>
        <p style='color:#4a6355; margin-bottom:16px; font-size:0.9rem'>Lihat tren historis dan proyeksi produktivitas padi 5 tahun ke depan per provinsi menggunakan model Facebook Prophet.</p>
    """, unsafe_allow_html=True)
    ...
    fig.update_layout(
        title=dict(text=f'Tren & Proyeksi — {provinsi_sel.capitalize()}', font=dict(size=13, color='#0f1c14')),
